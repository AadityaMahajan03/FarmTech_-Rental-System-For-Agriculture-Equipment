			...
			success : function(response){
				...
				if (resp.status == 202) {
					...
					$.each(resp.message, function(index, value){
						...
						customerOrderHTML +=`<tr><td>#</th><td>${value.order_id}</td><td>${value.product_id}</td><td>${value.product_title}</td><td>${value.qty}</td><td>${value.trx_id}</td><td>${value.p_status}</td><td>${value.cust_name}</td><td>${value.cust_num}</td></tr>`;
