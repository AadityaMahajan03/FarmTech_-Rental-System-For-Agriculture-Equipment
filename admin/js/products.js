$(document).ready(function(){
	...
	$(".add-product").on("click", function (e) {
		...
		var formData = new FormData($("#add-product-form")[0]); // Get all form data
		...
		$.ajax({
			...
			success: function (response) {
				...
				try {
					var resp = $.parseJSON(response); // Parse response JSON
	...
	$(document.body).on("click", ".edit-product", function () {
		...
		$("input[name='e_product_video']").val(product.product_video); // ✅ Add Video Field
